<section id="auspicio">
  <div class="container-fluid">
    <div class="container">
      <div class="row justify-content-md-center mt-5 text-center">

        <div class="col-md-3 col-6 offset-0 mb-2 d-flex align-items-center">
          <span class="logos_top" style="margin-top:-310px;">Organiza</span>
          <a href="https://www.instagram.com/aguacero.comics/" target="_blank">
            <img src="img/logos/aguacero.png" alt="" class="w-100 hvr-shrink" style="max-width:200px;">
          </a>
        </div>

        <div class="col-md-6 col-12 offset-0 mb-2 d-flex align-items-center">
          <span class="logos_top">Financia</span>
          <a href="https://www.cultura.gob.cl/" target="_blank">
            <img src="img/logos/ministerio_4.png" alt="" class="w-100" style="max-width:500px;">
          </a>
        </div>

      </div>


      <div class="row justify-content-md-center pt-5 text-center">

        <div class="col-md-6 col-12 offset-0 mb-2 d-flex align-items-center">
          <span class="logos_top" style="margin-top:-250px;">Auspicia</span>
          <a href="https://www.munivaldivia.cl/web/" target="_blank" class="me-5">
            <img src="img/logos/3.png" alt="" class="w-100 hvr-shrink" style="filter: brightness(0%);max-width:200px;">
          </a>
          <a href="https://www.prochile.gob.cl/" target="_blank">
            <img src="img/logos/prochile_gris.png" alt="" class="w-100 hvr-shrink" style="filter: brightness(0%);max-width:200px;">
          </a>
        </div>

        <div class="col-md-3 col-6 mb-2 d-flex align-items-center">
          <span class="logos_top" style="margin-top:-250px;">Colabora</span>
          <a href="https://www.bibliotecasantiago.gob.cl/" target="_blank">
            <img src="img/logos/20.png" alt="" class="w-100 hvr-shrink">
          </a>
        </div>
        <div class="col-md-3 col-6  mb-2 d-flex align-items-center">
          <span class="logos_top" style="margin-top:-250px;">Media Partner</span>
          <a href="https://www.bibliotecasantiago.gob.cl/" target="_blank">
            <img src="img/logos/13.png" alt="" class="w-100 hvr-shrink">
          </a>
        </div>

      </div>
    </div>
</section>