<!DOCTYPE html>
<html lang="en" dir="ltr">
  <head>
    <meta http-equiv="Expires" content="Mon, 22 Jul 2002 11:12:01 GMT">
    <meta http-equiv="Last-Modified" content="Mon, 22 Jul 2002 11:12:01 GMT">
    <meta http-equiv="Cache-Control" content="no-cache">
    <meta http-equiv="Pragma" content="no-cache">

    <meta charset="utf-8">
    <meta http-equiv="X-Frame-Options" content="allow-from https://www.youtube.com/">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <link rel="icon" type="image/x-icon" href="<?= base_url('img/favicon.png'); ?>">
    <link rel="apple-touch-icon" type="image/x-icon" href="<?= base_url('img/favicon.png'); ?>">
    <link rel="apple-touch-icon-precomposed" type="image/x-icon" href="<?= base_url('img/favicon.png'); ?>">
    <link rel="shortcut icon" type="image/x-icon" href="<?= base_url('img/favicon.png'); ?>">


    <!-- Primary Meta Tags -->
    <title>Aguacero Festival Internacional de Valdivia 2023</title>
    <meta name="title" content="Aguacero Festival Internacional de Valdivia 2023">
    <meta name="description" content="Aguacero Festival Internacional de Cómics de Valdivia, tiene como prioridad fomentar la lectura en los niños y jóvenes de la Región de Los Ríos, a través del lenguaje de la Narrativa Gráfica o Cómic.">

    <!-- Open Graph / Facebook -->
    <meta property="og:type" content="website">
    <meta property="og:url" content="https://festival.aguacerocomics.cl/">
    <meta property="og:title" content="Aguacero Festival Internacional de Valdivia 2023">
    <meta property="og:description" content="Aguacero Festival Internacional de Cómics de Valdivia, tiene como prioridad fomentar la lectura en los niños y jóvenes de la Región de Los Ríos, a través del lenguaje de la Narrativa Gráfica o Cómic.">
    <meta property="og:image" content="<?= base_url('img/saval.jpg'); ?>">

    <!-- Twitter -->
    <meta property="twitter:card" content="summary_large_image">
    <meta property="twitter:url" content="https://festival.aguacerocomics.cl/">
    <meta property="twitter:title" content="Aguacero Festival Internacional de Valdivia 2023">
    <meta property="twitter:description" content="Aguacero Festival Internacional de Cómics de Valdivia, tiene como prioridad fomentar la lectura en los niños y jóvenes de la Región de Los Ríos, a través del lenguaje de la Narrativa Gráfica o Cómic.">
    <meta property="twitter:image" content="<?= base_url('img/saval.jpg'); ?>">
  <!--
    <link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,400;0,500;0,700;1,400;1,500;1,700&display=swap" rel="stylesheet">
-->
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Roboto+Condensed:ital,wght@0,300;0,400;0,700;1,300;1,400;1,700&display=swap" rel="stylesheet">
    
    <link rel="stylesheet" type="text/css" href=<?= base_url("public/dist/bootstrap-5.1.3/css/bootstrap.css"); ?>>
    <link rel="stylesheet" type="text/css" href=<?= base_url("dist/fontawesome-6.0.0/css/all.css?v=0.1"); ?>>
    <link rel="stylesheet" type="text/css" href=<?= base_url("css/animate.css?v=0.1"); ?>>
    <link rel="stylesheet" type="text/css" href=<?= base_url("css/hover.css"); ?>>
    <link rel="stylesheet" type="text/css" href=<?= base_url("dist/magnificpopup-1.1.0/magnific-popup.css"); ?>>

    <link rel="stylesheet" type="text/css" href="<?= base_url('dist/datatables-1.11.3/datatables.min.css?v=0.01');?>">
    <link rel="stylesheet" type="text/css" href="<?= base_url('dist/datatables-1.11.3/Responsive-2.2.9/css/responsive.bootstrap5.css');?>">

    <link rel="stylesheet" type="text/css" href="<?= base_url('dist/flipdown/flipdown.css?v=0.2');?>">

    <link rel="stylesheet" type="text/css" href=<?= base_url("css/style.css?v=2.2"); ?>>

    <script type="text/javascript" src="<?= base_url('js/jquery-3.6.0.js');?>"></script>
    
    <!-- Scroll reveal -->
    <script type="text/javascript" src="https://unpkg.com/scrollreveal"></script>

    <!-- Slick carousel -->
    <script type="text/javascript" src="<?= base_url("dist/slick-1.8.1/slick.js");?>"></script>
    <link rel="stylesheet" type="text/css" href="<?= base_url("dist/slick-1.8.1/slick.css");?>"/>
    <link rel="stylesheet" type="text/css" href="<?= base_url("dist/slick-1.8.1/slick-theme.css");?>">

    

  </head>
  <body>
